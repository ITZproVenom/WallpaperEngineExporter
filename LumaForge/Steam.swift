...
struct SteamOpenID {
    ...
    static func steamID(from callback: URL, expectedState: String) -> String? {
        ...
        let q = Dictionary(uniqueKeysWithValues: (c.queryItems ?? []).compactMap { item in item.value.map { (item.name, $0) } })
        guard q["state"] == expectedState, q["openid.mode"] == "id_res",
              q["openid.op_endpoint"] == endpoint else { return nil }
...
final class SteamKeychain {
    ...
    func save(_ id: String) {
        let query: [String: Any] = [kSecClass as String:kSecClassGenericPassword,kSecAttrService as String:service,kSecAttrAccount as String:"steamID",kSecValueData as String:Data(id.utf8)]
    ...
    func clear() {
        SecItemDelete([kSecClass as String:kSecClassGenericPassword,kSecAttrService as String:service,kSecAttrAccount as String:"steamID"] as CFDictionary)
    }
}
...
final class SteamSession: NSObject, ObservableObject, ASWebAuthenticationPresentationContextProviding {
    ...
    func signIn() {
        ...
        var c = URLComponents(string: SteamOpenID.endpoint)!
        c.queryItems = [
        ...
        guard let url=c.url else { signingIn=false; return }
        let session=ASWebAuthenticationSession(url:url, callbackURLScheme:"lumaforge") { [weak self] callback,error in
            Task { @MainActor in
                ...
                guard error == nil, let callback, let id=SteamOpenID.steamID(from:callback,expectedState:self.state) else { return }
                do {
                    guard try await self.verify(callback) else { return }
                    self.steamID=id; self.keychain.save(id)
                } catch {}
            }
        }
    ...
    private func verify(_ callback: URL) async throws -> Bool {
        guard var c=URLComponents(url:callback,resolvingAgainstBaseURL:false) else { return false }
        var items=c.queryItems ?? []
        items.removeAll { $0.name == "openid.mode" }
        items.append(.init(name:"openid.mode",value:"check_authentication"))
        c.queryItems=items
