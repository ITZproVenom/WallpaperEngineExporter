                ...
                VStack(alignment: .leading, spacing: 18) {
                    HStack {
                        VStack(alignment: .leading) {
                            ...
                            Text(steam.steamID == nil ? "Wallpaper Engine Workshop" : "Steam connected")
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                Text("Workshop ID \(item.id)")
                    .foregroundStyle(.secondary)
                Text("Steam controls Workshop installation. LumaForge handles inspection and export after the package is available in Files.")
                    .foregroundStyle(.secondary)
...
struct LibraryView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    Button { pick = true } label: {
                        Label("Import from Files", systemImage: "doc.badge.plus")
                    }
                    Text("Import a Workshop package, project media, image, or video.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
        ...
        NavigationStack {
            List {
                if store.records.isEmpty {
                    ContentUnavailableView("No exports", systemImage: "arrow.down.circle")
                } else {
